...
import fennvilleContent from '@/data/fennville_content.json';
...
const determineBestBars = () => {
    ...
    const datasets = [
        [10, 6.92, 6.29, 10, 7.59, 10, 8, 7.16, 6.64, 3.68, 9.78, 6, 2.72, 4.97],
        [10, 7.22, 5.41, 10, 8.82, 10, 8, 7.62, 3.64, 5.27, 9.58, 6, 1.67, 4.85],
        [10, 6.93, 6.22, 10, 7.43, 10, 8, 6.09, 4.98, 5.36, 9.72, 6, 2.35, 5.04]
    ];
    ...
    const bestBars = categories.map((_, i) => {
        ...
        return bestIndexes.length > 1 ? 0 : bestIndexes[0];
    });
    ...
    datasets: [
        {
            ...
            backgroundColor: bestBars.map((best) => best === 0 ? 'rgba(212, 185, 219, 1)' : 'rgba(212, 185, 219, 0.1)'),
            borderRadius: 6,
        },
        {
            label: 'Berrien',
            data: [10, 7.22, 5.41, 10, 8.82, 10, 8, 7.62, 3.64, 5.27, 9.58, 6, 1.67, 4.85],
            backgroundColor: bestBars.map((best) => best === 1 ? 'rgba(137, 192, 245,1)' : 'rgba(137, 192, 245,0.1)'),
            borderRadius: 6,
        },
        {
            label: 'Van Buren',
            data: [10, 6.93, 6.22, 10, 7.43, 10, 8, 6.09, 4.98, 5.36, 9.72, 6, 2.35, 5.04],
            backgroundColor: bestBars.map((best) => best === 2 ? 'rgba(255, 229, 153,1)' : 'rgba(255, 229, 153,0.1)'),
            borderRadius: 6,
        },
    ],
};
        ...
        <Box sx={{ display: 'flex', justifyContent: 'center', width: '100%' }}>
            <Box sx={{
                ...
                <Box sx={{
                    ...
                    <Bar data={data} options={chartOptions} sx={{minHeight: '80%'}} />
                </Box>
                ...
                <Box sx={{
                    ...
                    <Link
                        href="https://www.mdpi.com/2073-4395/11/4/339"
                        target="_blank"
                        rel="noopener"
                        variant="body3"
                        sx={{ color: 'grey', textDecoration: 'none' }}
                    >
                        Graph produced by fincuva with data from 'Modeling Land Suitability for Vitis vinifera in Michigan Using Advanced Geospatial Data and Methods' by Wanyama D, Bunting EL, Goodwin R, Weil N, Sabbatini P, Andresen JA, © 2020 by the authors. Licensed under CC. https://www.mdpi.com/2073-4395/11/4/339
                    </Link>
                </Box>
